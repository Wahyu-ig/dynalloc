...
| **system** | `plugins/system.js` | System health monitoring plugin — flags active processes under high load or critical RAM. |
| **kde-wayland** | `plugins/kde-wayland.js` | Foreground detection for KDE Plasma Wayland via KWin DBus (qdbus). Auto-activates only on KDE Plasma Wayland sessions; returns `[]` on GNOME, Hyprland, Sway, or X11. Caches the foreground PID for 500ms to avoid DBus spam. Exposes `getForegroundPid()` for foreground source integration. |
