...
## [2.1.0] — 2026-07-14 — Tier 1 Killer Features

This release introduces **four new subsystems that no other Linux resource
daemon has today**. All four are opt-in (defaults OFF) and degrade gracefully
when their hardware/dependency prerequisites are not met. Together they
transform DynAlloc from a reactive scheduler into a predictive, energy-aware,
VRAM-aware, power-budget-aware system manager.

### Added — Tier 1 Killer Features

#### 1. Predictive Pre-Allocation Engine (`intelligence/predictive-engine.js`)
...
resource manager suffers from (GameMode, system76-scheduler, ananicy,
etc. are all purely reactive).
...
- Confidence threshold (default 0.45) gates pre-allocation
- Hold period (default 8000ms) — auto-reverts if prediction misses
- Cooldown (default 15000ms) prevents pre-allocation spam
- Handoff: when predicted app actually launches, the daemon's normal
  foreground boost takes over silently (no revert)
- Snapshot/rollback: every pre-allocation is paired with a revert timer
- Memory-bounded: LRU eviction at MAX_TRACKED_APPS × MAX_TRANSITIONS_PER_APP

#### 2. Per-App Energy Accounting via RAPL (`lib/energy-accountant.js`)

Reads Intel/AMD RAPL (Running Average Power Limit) energy counters from
`/sys/class/powercap/intel-rapl/` (Intel) or `/sys/class/hwmon/hwmonN/power`
(AMD) and **attributes joules to individual PIDs/cgroups based on CPU time
share**. Output: "Discord: 47 Wh this week, Blender: 312 Wh, Chrome: 89 Wh."

No other Linux resource daemon does per-app energy attribution today.
Windows + macOS both ship it in their battery UI; Linux users currently
have to dig through `powerstat` / `turbostat` output, which gives only
system-wide numbers.

**Counters tracked:** package, core, uncore, dram, psys (Intel) /
package (AMD, integrated via hwmon μW × Δt).

- 32-bit overflow detection (RAPL counters wrap ~every 60s at 100W)
- Memory-bounded LRU ledger (MAX_APPS = 256)
- History ring buffer (360 samples ≈ 6 minutes at 1Hz)
- Per-app queries: top consumers, joules, watt-hours, share
- System-wide: current watts, average watts, cumulative totals

#### 3. CPU+GPU Shared Power Budget Arbiter (`lib/controllers/power-budget-arbiter.js`)

Treats CPU + GPU as **one shared power budget**. When total draw exceeds
the budget, the arbiter decides who to throttle based on utilization
(GPU-bound vs CPU-bound vs balanced) — instead of letting firmware pick
the wrong one to throttle. All existing Linux daemons (GameMode, TLP,
power-profiles-daemon, LACT, CoreCtrl) manage CPU and GPU **independently**.

**Decision tree:**
```
total = cpu_power + gpu_power
if total > budget * 0.95:
  if gpu_bound (gpu > 80%, cpu < 30%):    cap CPU, preserve GPU
  elif cpu_bound (cpu > 60%, gpu < 40%):  cap GPU, preserve CPU
  else:                                    split the cut (both capped)
```

- Hysteresis: 0.95× headroom to engage, 1.10× to release (prevents oscillation)
- CPU min floor (default 8W), GPU min floor (default 10W) — never starves
- Snapshot/rollback via GpuController's existing mechanism
- Tick interval: 2000ms (configurable)
- DRY_RUN aware

#### 4. GPU VRAM Pressure Reclaim Controller (`lib/controllers/vram-reclaim-controller.js`)

Monitors VRAM usage. When VRAM > 92% AND foreground needs VRAM,
**briefly SIGSTOPs the largest non-foreground VRAM hogs** so the driver
reclaims their VRAM. Prevents GPU OOM crashes that plague Linux gaming
when 50 browser tabs + Discord + a Steam game launch simultaneously.

There is currently NO Linux daemon that proactively reclaims VRAM from
background processes BEFORE the foreground app fails its allocation.

**Detection signals (any one triggers reclaim):**
1. VRAM usage > HIGH_WATERMARK (default 92%)
2. dmesg contains GPU OOM / VRAM allocation failure patterns
3. Foreground app's GPU utilization drops sharply while GPU is still busy

**Candidate selection:**
- `nvidia-smi --query-compute-apps=pid,used_memory` (NVIDIA, exact)
- `/proc/[pid]/maps` scan for `[drm]` / `amdgpu` backed maps (AMD fallback)
- Foreground PID + critical process patterns always skipped
- Top N candidates (default 2) suspended per tick
- SIGSTOP hold: 3000ms (configurable)
- Per-PID cooldown: 30000ms (prevents suspend-spam)
- NEVER kills — only briefly pauses

**Critical process patterns** (always skipped): Xorg, Xwayland, gnome-shell,
mutter, kwin_*, plasmashell, pipewire, pulseaudio, systemd, dbus, sddm, gdm,
steam, gamescope, gamemoded.

### New CLI Commands

- `dynalloc predict [--app <name>] [--top] [--limit N]` — Show predictive
  pre-allocation status and predictions
- `dynalloc energy [--apps] [--history] [--limit N]` — Show per-app energy
  accounting (RAPL)
- `dynalloc vram [--force-release]` — Show GPU VRAM reclaim status, force-
  release any suspended PIDs
- `dynalloc powerbudget [--force-release]` — Show CPU+GPU power budget
  arbiter status, force-release any active cap

### New IPC Handlers

- `predict` — query predictions, top transitions, status
- `energy` — query per-app joules, system watts, totals
- `vram` — query VRAM reclaim status, force-release
- `powerbudget` — query power budget status, force-release

### New Policy Engine Action Types

- `setPowerBudget` — release/engage/set-budget for the CPU+GPU arbiter
- `reclaimVram` — release/status for VRAM reclaim
- `preAllocate` — informational (use IPC `predict` for actual inspection)

### New Configuration Keys

| Key | Default | Description |
|-----|---------|-------------|
| `ENABLE_PREDICTIVE_ENGINE` | `false` | Master switch for predictive pre-allocation |
| `PREDICTION_CONFIDENCE_THRESHOLD` | `0.45` | Minimum confidence to trigger pre-allocation |
| `PREDICTION_HOLD_MS` | `8000` | Pre-allocation auto-revert timeout |
| `PREDICTION_COOLDOWN_MS` | `15000` | Cooldown between pre-allocations |
| `ENABLE_ENERGY_ACCOUNTING` | `false` | Master switch for RAPL energy accounting |
| `ENERGY_ACCOUNTING_TICK_MS` | `1000` | Energy sampling interval |
| `ENABLE_POWER_BUDGET_ARBITER` | `false` | Master switch for CPU+GPU power budget |
| `POWER_BUDGET_TOTAL_WATTS` | `65` | Total system power budget (W) |
| `POWER_BUDGET_CPU_MIN_WATTS` | `8` | CPU floor when capped |
| `POWER_BUDGET_GPU_MIN_WATTS` | `10` | GPU floor when capped |
| `POWER_BUDGET_HEADROOM_FACTOR` | `0.95` | Engage cap at 95% of budget |
| `POWER_BUDGET_RELEASE_FACTOR` | `1.10` | Release cap at 110% of budget |
| `POWER_BUDGET_TICK_MS` | `2000` | Arbiter re-evaluation interval |
| `ENABLE_VRAM_RECLAIM` | `false` | Master switch for GPU VRAM reclaim |
| `VRAM_RECLAIM_TICK_MS` | `5000` | VRAM scan interval |
| `VRAM_RECLAIM_HIGH_WATERMARK` | `0.92` | VRAM % used → trigger reclaim |
| `VRAM_RECLAIM_LOW_WATERMARK` | `0.80` | VRAM % used → clear pressure state |
| `VRAM_RECLAIM_HOLD_MS` | `3000` | SIGSTOP hold duration |
| `VRAM_RECLAIM_COOLDOWN_MS` | `30000` | Per-PID cooldown |
| `VRAM_RECLAIM_MAX_PER_TICK` | `2` | Max PIDs suspended per tick |

### Daemon Integration

- New bootstrap step 16l: constructs all four subsystems after
  Intelligence Subsystem (16k) and before IPC server (17)
- `handleForegroundChange()` now feeds the PredictiveEngine with
  transitions + tries to pre-allocate for the next predicted app
- `getState()` now includes predictiveEngine, energyAccountant,
  powerBudgetArbiter, vramReclaimController status snapshots
- `cleanupAndExit()` reverts pre-allocations, resumes suspended PIDs,
  releases power caps BEFORE destroying the resource controller manager

### Tests

- **`test/unit/test-predictive-engine.js`** — 26 tests: construction,
  transition recording, prediction (confidence, threshold gating),
  preAllocate lifecycle, cooldown, handoff on hit, auto-revert on
  timeout, forceRevert, status, topTransitions, hot-reload, memory bounds
- **`test/unit/test-energy-accountant.js`** — 24 tests: construction,
  setup/vendor probe, sampling (deltaJoules, attribution, zero/invalid
  share), 32-bit overflow handling, queries (top apps, recent watts,
  totals, average), status, reset, hot-reload, memory bounds
- **`test/unit/test-power-budget-arbiter.js`** — 24 tests: inheritance,
  capability model, decision tree (gpu_bound/cpu_bound/balanced), engage/
  release/adjust, min floors, status, hot-reload, lifecycle hooks
- **`test/unit/test-vram-reclaim-controller.js`** — 29 tests: inheritance,
  capability model, VRAM reads (DRY_RUN), candidate selection, never-
  suspend list, cooldown, suspend/resume lifecycle, forceRelease,
  status, hot-reload

All 103 new tests run in DRY_RUN mode — no real syscalls, sysfs writes,
SIGSTOP/SIGCONT, or external process spawning.

### Backward Compatibility

All four features default OFF. When their master switches are false:
- No subsystem is constructed
- No /sys or /proc paths are read
- No SIGSTOP/SIGCONT is sent
- The daemon behaves exactly like v2.0.0

When enabled but prerequisites are missing (e.g.ENABLE_ENERGY_ACCOUNTING
is true but RAPL is not readable), the subsystem reports
`isAvailable() === false` and becomes inert — no errors, no crashes.

### Version Bump

- `package.json` version: 2.0.0 → 2.1.0
- `dynalloc-cli.js` VERSION: 2.0.0 → 2.1.0
- Banner: "DynAlloc v2.1 — Adaptive Linux Resource Manager"

## [Unreleased]

### Added

- **GPU Controller** (`lib/controllers/gpu-controller.js`): a new
  Resource Controller Layer domain, sitting alongside the existing
  Thermal/Power controllers. Adds a write path on top of the
  pre-existing read-only GPU utilization awareness
  (`sensor.js#getGpuUtilization`):
  - **NVIDIA**: power limit control via `nvidia-smi -pl <watts>`,
    clamped to the card's own `[power.min_limit, power.max_limit]`
    range. `balanced` restores `power.default_limit`, `power-saver`
    targets ~60% of `power.max_limit`, `performance` targets
    `power.max_limit`.
  - **AMD**: `power_dpm_force_performance_level` sysfs write
    (`auto` / `low` / `high`).
  - **Intel**: no safe universal write path — the controller reports
    `isAvailable() === false` and read-only awareness is unaffected.
  - New config keys (all default OFF/safe): `ENABLE_GPU_CONTROL`
    (master switch, default `false`), `GPU_PROFILE_DEFAULT`
    (default `'balanced'`), `GPU_USE_SUDO` (default `false`, mirrors
    `GOVERNOR_USE_SUDO`'s `sudo -n` convention).
  - New Policy Engine action type: `setGpuProfile` (e.g.
    `{ "type": "setGpuProfile", "profile": "performance" }"`),
    routed through `ResourceControllerManager.applyGpuProfile()`.
  - Snapshot/rollback support via `restoreSnapshot()`, same contract
    as `ThermalController`/`PowerController`.
  - Fully additive and backward compatible: `ENABLE_GPU_CONTROL`
    defaults to `false`, so no daemon starts touching GPU power
    limits without explicit opt-in. Existing GPU-bound workload
    classification and read-only telemetry are untouched.
  - 25 new unit tests in `test/unit/test-gpu-controller.js`
    (capability model, NVIDIA/AMD profile math, sudo command
    building, snapshot/rollback, `ResourceControllerManager` +
    `ActionExecutor` wiring). All run in `DRY_RUN` — no real
    syscalls or sysfs writes.

## [2.0.0] — 2026-07-13 — Intelligence, Diagnostics & Transparency

This is the largest feature release since v1.0.0. It introduces a complete
intelligence subsystem focused on behavioral learning, explainability,
diagnostics, and transparency — without any external dependencies or
machine learning libraries. All new features are deterministic, lightweight,
and fully backward-compatible.

### Added — Intelligence Subsystem (`intelligence/`)

A new top-level module containing six components:

#### 1. Learning Engine (`intelligence/learning-engine.js`)

A lightweight statistical learning engine that observes daemon behavior
and builds frequency-based profiles. No ML libraries — purely
deterministic frequency counting, time-windowed statistics, and simple
heuristics.

**What it learns:**
- Frequently used applications (by foreground time)
- Common active hours (hour-of-day histogram, 0-23)
- Profile switching patterns (trigger → profile mappings)
- Battery usage habits (discharge rate tracking with anomaly detection)
- CPU intensive workflows (high-pressure foreground apps)
- Foreground application history (ring buffer, most recent 5000 entries)

**Key properties:**
- Memory-bounded: all internal stores have fixed max sizes (LRU eviction)
- Non-blocking: all operations are O(1) or O(k) with small k
- Deterministic: no randomness, fully reproducible
- Persistent: learning data is saved to `~/.config/dynalloc/learning/learning-state.json`
- Explainable: every learned pattern can be traced back to observations

#### 2. Recommendation Engine (`intelligence/recommendation-engine.js`)

Generates actionable suggestions based on observed patterns from the
LearningEngine. **Recommendations are NEVER applied automatically** —
they always require explicit user approval via the CLI.

**Recommendation types:**
- `auto-profile`: "Steam is usually associated with Gaming profile (12 times). Create a policy?"
- `battery-habit`: "You frequently switch to Battery Saver at 10% battery. Create a permanent policy?"
- `cpu-intensive`: "Chrome consistently causes high CPU pressure (avg: 55.3%). Create a throttle rule?"
- `time-based`: "You often use Gaming apps at 19:00. Schedule a profile switch?"
- `anomaly`: "Battery discharge rate is unusual. Check for background processes?"

**Lifecycle:** pending → approved (by user) → dismissed (with cooldown)

#### 3. Explainability Engine (`intelligence/explainability-engine.js`)

Every automatic decision now includes a structured explanation. Each
explanation contains:
- **Decision**: what was decided (boost, throttle, governor_change, policy_rule, profile_switch)
- **Target**: which process or resource was affected
- **Factors**: a list of check/result pairs showing WHY each factor contributed
- **Outcome**: final result description
- **Context**: system state at the time (CPU pressure, battery, thermal, etc.)

Example output:
```
Decision: boost
Target: steam (PID 1234)
Source: scheduler
Reason:
  ✓ Application classified as INTERACTIVE
  ✓ CPU pressure 10.2% acceptable (10.2)
  ✓ Memory pressure 5.1% acceptable (5.1)
  ✓ Battery 85% sufficient (85)
  ✓ CPU temperature 65.0°C safe (65)
  ✓ Pinned to cores [0, 1, 2]
  ✓ Nice set to -5
  ✓ IO priority class 2, level 0
  ✓ Governor changed to performance
Outcome: Full boost applied
```

#### 4. Timeline Engine (`intelligence/timeline-engine.js`)

An internal event timeline that records all important daemon events
for diagnostics, debugging, and the HTML report. Events are queryable
by the CLI.

**Event categories:** daemon, scheduler, governor, policy, profile,
plugin, system, learning, focus, adaptive.

**Query support:** category, event, severity, time range, full-text search,
pagination.

**Memory-bounded:** ring buffer of 10,000 entries (configurable).

#### 5. Doctor Engine (`intelligence/doctor-engine.js`)

A comprehensive health check that replaces the basic self-check with
18 verification points covering:

- System Compatibility (kernel version, architecture, privileges)
- PSI Availability (CPU, Memory, I/O pressure stall info)
- CPU Governor (cpufreq interface, PPD coordination)
- Power Profiles Daemon
- Required Binaries (renice, ionice, taskset)
- Optional Binaries (cpupower, notify-send, hyprctl, swaymsg, etc.)
- Configuration (threshold validation, tick interval checks)
- File Permissions (config dir, cgroup access, /proc access)
- D-Bus Session (desktop environment detection)
- Scheduler (running, stress level)
- Policy Engine (rules loaded, running)
- Plugin System (plugins loaded)
- Event Bus (active bus detected)
- IPC (connection successful)
- Rule Validation
- Plugin Integrity
- Learning Engine
...
Each check returns **PASS**, **WARNING**, or **ERROR**. An overall
health score (0-100%) is calculated.

#### 6. Report Generator (`intelligence/report-generator.js`)

Generates a standalone HTML diagnostic report. The report is:
- **Self-contained**: no external dependencies, no CDN, no internet needed
- **Dark-themed**: professional dark UI with CSS variables
- **Interactive**: category-filterable timeline, filterable by event type
- **Comprehensive**: system info, health score, performance summary,
  recommendations, explanations, timeline, health check details,
  and full current configuration

### New CLI Commands

- `dynalloc doctor` — Enhanced comprehensive health check with score
- `dynalloc report [file]` — Generate standalone HTML diagnostic report
- `dynalloc timeline [--category X] [--limit N] [--search Y]` — Query event timeline
- `dynalloc recommendations [--json]` — Show pending recommendations
- `dynalloc explanations [--type X] [--limit N]` — Show decision explanations
- `dynalloc intelligence [--json]` — Show intelligence subsystem status

### New IPC Commands

- `doctor` — Enhanced (uses DoctorEngine when available, falls back to self-check)
- `report` — Generates HTML report
- `timeline` — Queries the timeline
- `recommendations` — Lists/approves/dismisses recommendations
- `explanations` — Queries recent explanations
- `intelligence` — Intelligence subsystem status

### New Configuration Keys

| Key | Default | Description |
|-----|---------|-------------|
| `ENABLE_INTELLIGENCE` | `true` | Master switch for intelligence subsystem |
| `INTELLIGENCE_PERSIST_INTERVAL_MS` | `300000` | Learning data persistence interval (5 min) |
| `INTELLIGENCE_RECOMMENDATION_INTERVAL_MS` | `60000` | Recommendation generation interval (60s) |
| `LEARNING_SUGGESTION_COOLDOWN_MS` | `3600000` | Cooldown before re-suggesting (1h) |
| `LEARNING_DATA_DIR` | `null` | Directory for learning data (default: `~/.config/dynalloc/learning`) |
| `EXPLAINABILITY_ENABLED` | `true` | Attach explanations to all decisions |
| `TIMELINE_ENABLED` | `true` | Record all important events |
| `TIMELINE_MAX_ENTRIES` | `10000` | Ring buffer size for timeline |

### Daemon Integration

- `_executeBoost()` now records timeline events and explanations
- `_executeThrottle()` now records timeline events and explanations
- `handleForegroundChange()` now feeds the Learning Engine
- New subsystems are constructed in bootstrap step 16k (after Plugin SDK)
- New subsystems are cleaned up in `cleanupAndExit()`
- `getState()` now includes intelligence subsystem status
- Version bumped to 2.0.0

### Tests

- **`test/unit/test-intelligence.js`** — 37 comprehensive tests covering:
  - LearningEngine (10 tests): construction, foreground recording, CPU intensive tracking,
    hourly activity, top apps, battery samples, profile switches, persistence,
    memory bounds, status, clearing
  - RecommendationEngine (6 tests): construction, CPU-intensive generation,
    approve/dismiss, cooldown enforcement, status, no-auto-apply guarantee
  - ExplainabilityEngine (9 tests): boost/throttle/governor/policy/profile explanations,
    querying, ring buffer enforcement, text formatting, status
  - TimelineEngine (10 tests): recording, severity methods, category/severity/time/search
    queries, pagination, max entries, data export, stats
  - DoctorEngine (3 tests): score generation, binary checks, config issue detection
  - ReportGenerator (6 tests): HTML generation, health score rendering, timeline,
    recommendations, explanations, configuration, HTML entity escaping

### Architecture Documentation

- Updated `Architecture.md` with intelligence subsystem module dependency graph
- Updated data flow diagrams to show learning and explanation paths

## [1.1.0] — 2026-07-13 — Milestone: Watch, KDE Wayland Plugin, Expanded Tests

### Added — v1.1.0 Milestone

This release adds four major deliverables requested in the v1.1.0
milestone, with no breaking changes to the existing API.

#### 1. CI Pipeline Updates

The GitHub Actions CI pipeline (`.github/workflows/ci.yml`) now
runs every unit test in `test/unit/`, including the previously
missing `test-system-plugin.js` and the three new test suites added
in this release. The verify-scripts job also runs the new
`verify-v1_1_0.js` regression script. The pipeline fails fast on
any error and produces a clear summary in the GitHub Actions
summary panel.

- Added `test-system-plugin.js` to the unit-tests job (was present
  in the repo but not executed in CI).
- Added `test-learning-logger.js` (new in v1.1.0).
- Added `test-per-app-profiles.js` (new in v1.1.0).
- Added `test-kde-wayland-plugin.js` (new in v1.1.0).
- Added `scripts/verify-v1_1_0.js` to the verify-scripts job.
- Updated `package.json` `test` and `test:unit` scripts to include
  the new test files.
- Added per-job summary output via `$GITHUB_STEP_SUMMARY`.

#### 2. Expanded Test Coverage

Two new comprehensive unit test suites:

- **`test/unit/test-learning-logger.js`** — 65 tests covering the
  `LearningLogger` class: construction & defaults, `logAction()` for
  boost/throttle, invalid input (non-string action, null/undefined
  context), edge cases (empty context, non-numeric fields, special
  characters), error handling (read-only dirs, permission denied),
  file persistence (JSON-line format, append-only, re-read,
  malformed lines), `readEntries()` (empty/missing/whitespace
  files), `suggestRules()` (below/above threshold, boost/throttle
  grouping, sorting, threshold edge cases), `clear()` (idempotency,
  re-log after clear), getters, and round-trip integration.

- **`test/unit/test-per-app-profiles.js`** — 66 tests covering the
  `PerAppProfiles` class: construction & defaults, `getProfile()`
  normal behavior (match, no-match, multiple files), invalid input
  (non-string comm, empty string, feature flag disabled), field
  validation (non-string schedClass, non-number nice, etc.), file
  persistence (loading from disk, missing dir, malformed JSON,
  non-object files, mixed extensions), cache behavior (5-second
  reload window, force reload), helper methods (`isProtected()`,
  `getOverrideSchedClass()`, `getOverrideNice()`), getters, feature
  flag disabled, and round-trip integration.

- **`test/unit/test-kde-wayland-plugin.js`** — 71 tests for the new
  KDE Plasma Wayland plugin (see below).

#### 3. `dynalloc watch` Live Dashboard

A new CLI command that displays a live, ANSI-rendered dashboard
...
Features:
- **1-second refresh** (configurable via `--refresh <ms>`, minimum 100ms).
- **ANSI color rendering** — stress levels color-coded (green/yellow/red),
  governor color-coded, foreground process highlighted.
- **Anti-flicker rendering** — uses cursor-home + line padding rather
  than full-screen clear on every frame. The first frame clears the
  screen; subsequent frames overwrite in place. Lines are padded to
  terminal width to clear leftover characters.
- **Ctrl+C clean exit** — SIGINT handler restores the cursor and
  exits with code 0.
- **--width <cols>** option to override auto-detected terminal width.
- **Lightweight** — `setInterval` is `unref()`'d so the timer doesn't
  keep the event loop alive; the daemon CPU overhead from a single
  `watch` client is negligible (3 IPC calls + 4 /proc reads per second).

Displayed information:
- Daemon version, PID, uptime
- CPU usage %, load average, core count, CPU PSI pressure
- Memory used/total, memory PSI pressure
- Thermal state (if enabled)
- Scheduler stress level (NORMAL/WARN/CRITICAL), adaptive score, tick interval
- Active policy (if Policy Engine running)
- Current profile (if Profile Manager enabled)
- Governor (performance/powersave/schedutil/...)
- Foreground process (PID + comm)
- Boosted processes (count + list)
- Throttled processes (count + list)
- Loaded plugins
- Recent events (foreground changes, boosts, throttles, restores)

The renderer is decoupled from the CLI in `lib/watch-renderer.js`,
making it unit-testable without touching I/O.

#### 4. KDE Plasma Wayland Foreground Detection Plugin

A new plugin at `plugins/kde-wayland.js` that detects the foreground
window process on KDE Plasma Wayland sessions using KWin's DBus
interface (qdbus).

```
plugins/kde-wayland.js  — auto-loaded by PluginManager
```

Session detection (auto-activate, no config required):
- `XDG_CURRENT_DESKTOP` contains "KDE" (case-insensitive, supports
  multi-value strings like "ubuntu:KDE")
...
1. **Plasma 6**: `qdbus org.kde.KWin /org/kde/KWin/WindowManagement`
   `org.freedesktop.DBus.Properties.Get` → `ActiveWindow` →
   `GetWindowInfo` → "pid"
2. **Plasma 5.27+**: `qdbus org.kde.KWin /Scripting`
   `org.kde.kwin.Scripting.evaluateScript` "workspace.activeClient.pid"

Plugin interface (follows existing plugin conventions):
- `name`, `version`, `description` — standard manifest
- `init(config)` — initializes state, detects session
- `detect(procs, context)` — standard plugin interface; returns BOOST
  for the currently focused process so the scheduler's plugin layer
  excludes it from throttling
- `getForegroundPid()` — async foreground PID query (2s timeout per
  qdbus call, cached for 500ms to avoid DBus spam)
- `isSupported()` — synchronous session check
- `destroy()` — cleanup (no-op; plugin is stateless)

Backward compatibility & non-interference:
- On GNOME/Hyprland/Sway/X11 sessions, `detect()` returns `[]` and
  `getForegroundPid()` returns `null`. The plugin does NOT register
  any new IPC handlers, config keys, or daemon modules.
- The existing foreground detection chain in `sensor.js` (which also
  includes qdbus for KDE Wayland) is untouched. The plugin provides
  a parallel, plugin-layer integration point.
- All `execFile` calls use `qdbus` directly (no shell), with a strict
  2-second timeout. No command injection risk.
- All errors are caught and downgraded to "no foreground detected" —
  the plugin never throws.

### Changed

- `package.json` version: 1.0.0 → **1.1.0**
- `dynalloc-cli.js` VERSION: 1.0.0 → **1.1.0**
- `daemon.js` IPC ping version: 1.0.0 → **1.1.0**
- `package.json` `test` and `test:unit` scripts: now include all
  v1.1.0 test files.
- `.github/workflows/ci.yml` unit-tests job: now runs all 13 unit
  test suites (was 10).
- `.github/workflows/ci.yml` verify-scripts job: now runs 18 verify
  scripts (was 17).

### Added (Files)

- `lib/watch-renderer.js` — ANSI rendering module for `dynalloc watch`.
- `plugins/kde-wayland.js` — KDE Plasma Wayland foreground detection plugin.
- `test/unit/test-learning-logger.js` — 65 tests for `learning-logger.js`.
- `test/unit/test-per-app-profiles.js` — 66 tests for `per-app-profiles.js`.
- `test/unit/test-kde-wayland-plugin.js` — 71 tests for the new plugin.
- `scripts/verify-v1_1_0.js` — 78 assertions verifying all v1.1.0 deliverables.

### Validation Results

- **908/908 unit tests pass** (was 715; added 193 new tests)
- **115/115 integration tests pass** (unchanged)
- **18/18 verify scripts pass** (was 17; added verify-v1_1_0)
- **78/78 v1.1.0 verification assertions pass**
- Daemon boots in DRY_RUN mode with no errors
- All new files pass `node --check` syntax validation
