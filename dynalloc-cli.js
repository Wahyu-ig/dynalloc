...
// ── v2.1.0: Tier 1 killer feature CLI commands ─────────────────────

async function cmdPredict(socketPath, opts = {}) {
  const args = {};
  if (opts.app) args.currentApp = opts.app;
  if (opts.battery != null) args.battery = opts.battery;
  if (opts.top) args.top = true;
  if (opts.limit) args.limit = opts.limit;

  const res = await sendCommand(socketPath, 'predict', args);
  if (!res.ok) throw new Error(res.error);
  ...
  if (!res.data.enabled) {
    console.log('Predictive Engine: disabled');
    return;
  }
  ...
  console.log('='.repeat(60));
  const d = res.data;
  console.log(`  Threshold:    ${(d.threshold * 100).toFixed(1)}%`);
  console.log(`  Hold (ms):    ${d.holdMs}`);
  console.log(`  Cooldown(ms): ${d.cooldownMs}`);
  console.log(`  Transitions:  ${d.transitionsTracked}`);
  console.log(`  Contexts:     ${d.contextsTracked}`);
  if (d.activePreAllocation) {
    console.log(`  Active:       ${d.activePreAllocation.to} (${(d.activePreAllocation.confidence * 100).toFixed(1)}%)`);
  } else {
    console.log(`  Active:       none`);
  }
  const s = d.stats;
  console.log(`  Stats:`);
  console.log(`    Predictions:    ${s.predictions}`);
  console.log(`    Triggered:      ${s.predictionsTriggered}`);
  console.log(`    Hits:           ${s.predictionsHit}`);
  console.log(`    Misses:         ${s.predictionsMiss}`);
  console.log(`    Reverts:        ${s.reverts}`);

  if (d.topTransitions) {
    console.log(`  Top Transitions from "${opts.app}":`);
    for (const t of d.topTransitions) {
      const pct = (t.confidence * 100).toFixed(1);
      console.log(`    ${t.to.padEnd(20)} ${String(t.count).padStart(4)}/${t.total}  (${pct}%)`);
    }
  } else if (d.prediction) {
    const p = d.prediction;
    console.log(`  Prediction: ${p.to} (confidence ${(p.confidence * 100).toFixed(1)}%, count ${p.count}/${p.total})`);
  }
}

async function cmdEnergy(socketPath, opts = {}) {
  const args = {};
  if (opts.apps) args.apps = true;
  if (opts.history) args.history = true;
  if (opts.limit) args.limit = opts.limit;

  const res = await sendCommand(socketPath, 'energy', args);
  if (!res.ok) throw new Error(res.error);
  ...
  if (!res.data.enabled) {
    console.log('Energy Accountant: disabled');
    return;
  }

  console.log('Per-App Energy Accounting (RAPL)');
  console.log('='.repeat(60));
  const d = res.data;
  if (!d.available) {
    console.log('  RAPL not available on this system');
    return;
  }
  console.log(`  Vendor:       ${d.vendor}`);
  console.log(`  Domains:      ${d.domains.join(', ')}`);
  console.log(`  Samples:      ${d.samples}`);
  console.log(`  Apps tracked: ${d.appsTracked}`);
  console.log(`  History:      ${d.historySize}`);
  console.log(`  Current:      ${d.currentWatts !== null ? d.currentWatts.toFixed(2) + ' W' : 'n/a'}`);
  console.log(`  Average(60s): ${d.averageWatts !== null ? d.averageWatts.toFixed(2) + ' W' : 'n/a'}`);
  if (d.totals) {
    console.log(`  Totals (J):   pkg=${d.totals.package.toFixed(0)} core=${d.totals.core.toFixed(0)} uncore=${d.totals.uncore.toFixed(0)} dram=${d.totals.dram.toFixed(0)}`);
    if (d.totals.wattHours) {
      console.log(`  Totals (Wh):  pkg=${d.totals.wattHours.package} dram=${d.totals.wattHours.dram}`);
    }
  }
  if (d.apps) {
    console.log('');
    console.log('  Per-App Energy (top consumers):');
    console.log('    ' + 'App'.padEnd(24) + 'Joules'.padStart(12) + 'Wh'.padStart(10) + 'Share'.padStart(10) + 'CPUt'.padStart(10));
    for (const a of d.apps) {
      console.log('    ' + a.comm.padEnd(24) +
        a.joules.toFixed(1).padStart(12) +
        a.wattHours.toFixed(3).padStart(10) +
        (a.share * 100).toFixed(1).padStart(9) + '%' +
        String(Math.round(a.cpuTime)).padStart(10));
    }
  }
}

async function cmdVram(socketPath, opts = {}) {
  const args = {};
  if (opts.forceRelease) args.forceRelease = true;
  const res = await sendCommand(socketPath, 'vram', args);
  if (!res.ok) throw new Error(res.error);
  ...
  if (!res.data.enabled) {
    console.log('VRAM Reclaim Controller: disabled');
    return;
  }

  if (res.data.released) {
    console.log('All suspended PIDs resumed.');
    return;
  }

  console.log('GPU VRAM Pressure Reclaim Controller');
  console.log('='.repeat(60));
  const d = res.data;
  console.log(`  Available:    ${d.available}`);
  console.log(`  Vendor:       ${d.vendor}`);
  console.log(`  VRAM total:   ${d.vramTotalMB !== null ? d.vramTotalMB + ' MB' : 'n/a'}`);
  console.log(`  State:        ${d.state}`);
  console.log(`  High WM:      ${(d.highWatermark * 100).toFixed(0)}%`);
  console.log(`  Low WM:       ${(d.lowWatermark * 100).toFixed(0)}%`);
  console.log(`  Hold (ms):    ${d.holdMs}`);
  console.log(`  Cooldown(ms): ${d.cooldownMs}`);
  const s = d.stats;
  console.log(`  Stats:`);
  console.log(`    Scans:             ${s.scans}`);
  console.log(`    Pressure detected: ${s.pressureDetected}`);
  console.log(`    Reclaim attempts:  ${s.reclaimAttempts}`);
  console.log(`    PIDs suspended:    ${s.pidsSuspended}`);
  console.log(`    PIDs resumed:      ${s.pidsResumed}`);
  console.log(`    VRAM reclaimed:    ${(s.vramReclaimedBytes / 1024 / 1024).toFixed(0)} MB`);
  if (d.suspendedPids && d.suspendedPids.length > 0) {
    console.log(`  Currently Suspended:`);
    for (const p of d.suspendedPids) {
      console.log(`    pid=${p.pid} (${p.comm}) — ${p.vramMB}MB — resume in ${Math.max(0, p.resumeAt - Date.now())}ms`);
    }
  }
}

async function cmdPowerBudget(socketPath, opts = {}) {
  const args = {};
  if (opts.forceRelease) args.forceRelease = true;
  const res = await sendCommand(socketPath, 'powerbudget', args);
  if (!res.ok) throw new Error(res.error);
  ...
  if (!res.data.enabled) {
    console.log('Power Budget Arbiter: disabled');
    return;
  }

  if (res.data.released) {
    console.log('Active power budget cap released.');
    return;
  }

  console.log('CPU+GPU Shared Power Budget Arbiter');
  console.log('='.repeat(60));
  const d = res.data;
  console.log(`  Available:    ${d.available}`);
  console.log(`  State:        ${d.state}`);
  console.log(`  Budget:       ${d.budget} W`);
  console.log(`  CPU floor:    ${d.cpuMin} W`);
  console.log(`  GPU floor:    ${d.gpuMin} W`);
  console.log(`  Evaluations:  ${d.evaluations}`);
  console.log(`  Caps applied: ${d.capCount}`);
  console.log(`  Caps released:${d.releaseCount}`);
  if (d.activeSnapshot) {
    const snap = d.activeSnapshot;
    console.log(`  Active Cap:`);
    console.log(`    Decision:    ${snap.decision}`);
    console.log(`    Engaged at:  ${new Date(snap.engagedAt).toISOString()}`);
    console.log(`    CPU cap:     ${snap.cpuCap !== null ? snap.cpuCap.toFixed(1) + ' W' : 'n/a'}`);
    console.log(`    GPU cap:     ${snap.gpuCap !== null ? snap.gpuCap.toFixed(1) + ' W' : 'n/a'}`);
  }
}
...
Commands:
  ...
  predict [opts]         Show predictive pre-allocation status (v2.1)
                         --app <name>     Predict next app from this one
                         --top            Show top transition candidates
                         --limit <N>      Limit results (default 5)
  energy [opts]          Show per-app energy accounting (v2.1)
                         --apps           List top energy consumers
                         --history        Show recent watts history
                         --limit <N>      Limit results (default 20)
  vram                   Show GPU VRAM reclaim controller status (v2.1)
                         --force-release  Resume all suspended PIDs now
  powerbudget            Show CPU+GPU power budget arbiter status (v2.1)
                         --force-release  Release any active power cap
...
async function main() {
  ...
  let refreshMs = 1000;
  let watchWidth = null;
  // v2.1.0: per-command options
  let appFilter = null;
  let batteryFilter = null;
  let topFlag = false;
  let appsFlag = false;
  let historyFlag = false;
  let forceReleaseFlag = false;
  let limitNum = null;
  const positional = [];
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === '--socket' || a === '-s') {
      ...
      batteryFilter = parseInt(args[++i], 10);
    } else if (a === '--top') {
      topFlag = true;
    } else if (a === '--apps') {
      appsFlag = true;
      ...
      historyFlag = true;
    } else if (a === '--force-release' || a === '--release') {
      forceReleaseFlag = true;
    } else if (a === '--limit') {
      limitNum = parseInt(args[++i], 10);
    } else if (a === '--help' || a === '-h') {
      cmdHelp();
      process.exit(0);
  ...
  try {
    switch (command) {
      ...
      case 'intelligence':
        ...
        break;
      case 'predict':
        await cmdPredict(socketPath, opts);
        break;
      case 'energy':
        await cmdEnergy(socketPath, opts);
        break;
      case 'vram':
        await cmdVram(socketPath, opts);
        break;
      case 'powerbudget':
        await cmdPowerBudget(socketPath, opts);
        break;
      case 'help':
      case '--help':
      case '-h':
