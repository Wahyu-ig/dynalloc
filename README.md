...
### 🆕 v2.1.0 — Tier 1 Killer Features (no other Linux daemon has these)

* **Predictive Pre-Allocation ("Crystal Ball"):** Uses learned app-transition patterns to predict the next app you'll launch, and pre-warms CPU governor + GPU power BEFORE the app actually starts. Eliminates "first 5 second stutter" — every other Linux resource daemon is purely reactive.
* **Per-App Energy Accounting (RAPL):** Reads Intel/AMD RAPL counters and attributes joules to individual apps by CPU time share. Output: "Discord: 47 Wh this week, Chrome: 89 Wh." Windows + macOS ship this in their battery UI; Linux users have nothing comparable today.
* **CPU+GPU Shared Power Budget Arbiter:** Treats CPU + GPU as ONE power budget. When total draw exceeds the budget, decides who to throttle based on utilization (GPU-bound vs CPU-bound) instead of letting firmware pick the wrong one. GameMode, TLP, power-profiles-daemon, LACT, CoreCtrl all manage CPU and GPU independently.
* **GPU VRAM Pressure Reclaim:** Monitors VRAM usage. When > 92% AND foreground needs VRAM, briefly SIGSTOPs the largest non-foreground VRAM hogs so the driver reclaims their VRAM. Prevents GPU OOM crashes that plague Linux gaming when 50 browser tabs + Discord + a Steam game launch simultaneously.

All four features default OFF — opt-in via config.
