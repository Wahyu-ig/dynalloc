...
console.log('Verifying v2.1.9 packaging files...\n');
...
test('Fix #70a: Debian control file exists', () => {
  ...
  assert.ok(content.includes('Version: 0.2.1'), 'should have Version: 0.2.1');
...
test('Fix #72b: PKGBUILD has correct package metadata', () => {
  ...
  assert.ok(content.includes('pkgver=0.2.1'), 'should have pkgver=0.2.1');
...
test('Fix #73c: Makefile references correct version', () => {
  const content = fs.readFileSync(path.join(PKG_DIR, 'Makefile'), 'utf8');
  assert.ok(content.includes('VERSION ?= 0.2.1'), 'should default to version 0.2.1');
