...
test('Fix #28a: doctor-engine reports Sway and KDE/Wayland in D-Bus session check', () => {
  ...
  const result = engine.run({
    config: {},
    selfCheckReport: { sway: true, kdeWayland: true, foregroundTool: {} },
    daemonState: {},
  });
  const dbusCheck = result.checks.find(c => c.name === 'D-Bus Session');
  assert.ok(dbusCheck, 'doctor should include a D-Bus Session check');
  assert.ok(dbusCheck.message.includes('Sway'), 'doctor should check Sway');
  assert.ok(dbusCheck.message.includes('KDE/Wayland'), 'doctor should check KDE/Wayland');
});

test('Fix #28b: doctor-engine reports each foreground tool individually', () => {
  const DoctorEngineModule = require('../intelligence/doctor-engine');
  const DoctorEngine = DoctorEngineModule.DoctorEngine || DoctorEngineModule;
  const engine = new DoctorEngine();
  const result = engine.run({
    config: {},
    selfCheckReport: {
      foregroundTool: { hyprctl: false, swaymsg: false, qdbus: false, kdotool: false, xdotool: true, gdbus: true },
    },
    daemonState: {},
  });
  const names = result.checks.map(c => c.name);
  assert.ok(names.includes('Foreground tool: hyprctl'), 'doctor should list hyprctl');
  assert.ok(names.includes('Foreground tool: swaymsg'), 'doctor should list swaymsg');
  assert.ok(names.includes('Foreground tool: qdbus'), 'doctor should list qdbus');
  const hyprctlCheck = result.checks.find(c => c.name === 'Foreground tool: hyprctl');
  assert.ok(hyprctlCheck.message.includes('Hyprland IPC'), 'doctor should describe hyprctl');
});

test('Fix #28c: doctor-engine warns about missing foreground tools for the detected session', () => {
  const DoctorEngineModule = require('../intelligence/doctor-engine');
  const DoctorEngine = DoctorEngineModule.DoctorEngine || DoctorEngineModule;
  const engine = new DoctorEngine();
  const result = engine.run({
    config: {},
    selfCheckReport: {
      hyprland: true, sway: true, kdeWayland: true,
      foregroundTool: { hyprctl: false, swaymsg: false, qdbus: false, kdotool: false },
    },
    daemonState: {},
  });
  const errorMessages = result.checks.filter(c => c.status === 'ERROR').map(c => c.message);
  assert.ok(errorMessages.some(m => m.includes('hyprctl')), 'doctor should warn when hyprctl missing on Hyprland');
  assert.ok(errorMessages.some(m => m.includes('swaymsg')), 'doctor should warn when swaymsg missing on Sway');
  assert.ok(errorMessages.some(m => m.includes('qdbus') && m.includes('kdotool')), 'doctor should warn when qdbus+kdotool missing on KDE/Wayland');
});
