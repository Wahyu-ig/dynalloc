...
test('Fix #50c: checkPowerProfilesDaemon does not throw on systems without PPD', () => {
  delete require.cache[require.resolve('../sensor')];
  const sensor = require('../sensor');
  // In the test environment, PPD is almost certainly not running.
  // The function should return {active: false} without throwing.
  const result = sensor.checkPowerProfilesDaemon();
...
test('Fix #55a: CLI doctor checks Power Profiles Daemon', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'dynalloc-cli.js'), 'utf8');
  assert.ok(src.includes("['Power Profiles Daemon'"),
    'doctor should check Power Profiles Daemon');
  assert.ok(src.includes('report.powerProfilesDaemon'),
    'doctor should reference report.powerProfilesDaemon');
});

test('Fix #55b: CLI doctor shows PPD diagnostic when active', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'dynalloc-cli.js'), 'utf8');
  assert.ok(/report\.powerProfilesDaemon && report\.powerProfilesDaemon\.active/.test(src),
    'doctor should check if PPD is active for diagnostic');
  assert.ok(src.includes('DynAlloc governor switching is muted'),
    'doctor should mention governor switch is muted when PPD active');
  assert.ok(src.includes('ENABLE_PPD_COORDINATION=false to override'),
