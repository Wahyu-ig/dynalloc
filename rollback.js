    ...
    try {
      ...
      for (const pidStr of Object.keys(saved.modifiedPids)) {
        ...
        if (typeof procState.nice === 'number') {
          try {
            execFileSync('renice', ['-n', String(procState.nice), '-p', String(pid)], { stdio: 'ignore', timeout: 3000 });
          } catch (_) { /* might fail */ }
        }
        ...
        if (Array.isArray(procState.ionice)) {
          try {
            execFileSync('ionice', ['-c', String(procState.ionice[0]), '-n', String(procState.ionice[1]), '-p', String(pid)], { stdio: 'ignore', timeout: 3000 });
          } catch (_) { /* might fail */ }
        }
