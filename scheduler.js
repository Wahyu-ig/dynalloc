...
function calculateAdaptiveScore(factors, config) {
  ...
  const cpuNorm = Math.min(factors.cpuPressure / (config.PSI_CPU_CRITICAL || 50), 1.0);
  score += cpuNorm * (config.ADAPTIVE_WEIGHT_CPU || 0.4);
  maxPossible += (config.ADAPTIVE_WEIGHT_CPU || 0.4);

  const memNorm = Math.min(factors.memPressure / (config.PSI_MEM_CRITICAL || 30), 1.0);
  score += memNorm * (config.ADAPTIVE_WEIGHT_MEM || 0.8);
  maxPossible += (config.ADAPTIVE_WEIGHT_MEM || 0.8);

  const fgActive = factors.hasForeground ? 1 : 0;
  score += fgActive * (config.ADAPTIVE_WEIGHT_FOREGROUND || 0.5);
  maxPossible += (config.ADAPTIVE_WEIGHT_FOREGROUND || 0.5);

  // Media penalty reduces score (media playing = less urgency to throttle)
  if (factors.mediaPlaying) {
    score -= (config.ADAPTIVE_WEIGHT_MEDIA || 1.2);
  }
  maxPossible += (config.ADAPTIVE_WEIGHT_MEDIA || 1.2);

  if (factors.onBattery) {
    score -= 0.5 * (config.ADAPTIVE_WEIGHT_BATTERY || 0.3);
  }
  maxPossible += (config.ADAPTIVE_WEIGHT_BATTERY || 0.3);

  const thermalNorm = factors.thermalTemp !== null
    ? Math.min(Math.max(factors.thermalTemp - 50, 0) / 30, 1.0)
    : 0;
  score += thermalNorm * (config.ADAPTIVE_WEIGHT_THERMAL || 0.4);
  maxPossible += (config.ADAPTIVE_WEIGHT_THERMAL || 0.4);
