...
const { setLevel: setLogLevel, info, warn, error, debug, fatal, setupFileLogging, closeFileLogging, setSchedulerStateProvider } = logger;
const { CpuHistory, readCpuPSI, readMemPSI, getForegroundPID, getProcessInfo, listProcesses, isGameModeActive, watchFocusChanges, readBatteryStatus, readThermalTemp, checkPowerProfilesDaemon, getGpuUtilization, getNetworkRxBytes } = require('./sensor');
...
// ── Intelligence Subsystem (v2.0, optional) ─────────────────────────
// Loaded lazily inside bootstrap() only when ENABLE_INTELLIGENCE is true.
// Provides: LearningEngine, RecommendationEngine, ExplainabilityEngine,
// TimelineEngine, DoctorEngine, ReportGenerator.
let IntelligenceModule = null;
...
// ── v2.0: Intelligence subsystem state ────────────────────────────
let learningEngine = null;
let recommendationEngine = null;
let explainabilityEngine = null;
let timelineEngine = null;
let doctorEngine = null;
let reportGenerator = null;
let intelligencePersistTimer = null;
let recommendationTimer = null;

// ── v2.1.0: Tier 1 killer feature state ───────────────────────────
let predictiveEngine = null;
let energyAccountant = null;
let energyAccountantTimer = null;
let powerBudgetArbiter = null;
let vramReclaimController = null;
let prevForegroundComm = null;
...
async function handleForegroundChange(fgPid) {
  ...
  // v2.0: Record foreground change to learning engine
  if (learningEngine) {
    learningEngine.recordForeground({
      pid: fgPid,
      comm: newInfo ? newInfo.comm : '',
  ...
  // v2.0: Record focus change to timeline
  if (timelineEngine) {
    timelineEngine.info('focus', 'foreground_changed',
      `Foreground: ${newInfo ? newInfo.comm : 'PID ' + fgPid}`,
      { pid: fgPid, comm: newInfo ? newInfo.comm : '', prevPid: prevPid });
  }

  // v2.1.0: Feed PredictiveEngine — record transition + try to pre-allocate
  if (predictiveEngine && newInfo && newInfo.comm) {
    try {
      predictiveEngine.recordTransition({
        from: prevForegroundComm,
        to: newInfo.comm,
        battery: null,  // battery info not always available in this scope
      });
      // Try to pre-allocate for the NEXT predicted app
      predictiveEngine.tryPreAllocate({
        currentApp: newInfo.comm,
        battery: null,
      });
    } catch (_) { /* non-fatal */ }
    prevForegroundComm = newInfo.comm;
  }
}
...
function _executeBoost(boost) {
  ...
  // v2.0: Record timeline event and explanation
  if (timelineEngine) {
    timelineEngine.info('scheduler', 'boost', `Boosted ${boost.comm || 'PID ' + boost.pid}`, { pid: boost.pid, comm: boost.comm });
  }
  if (explainabilityEngine) {
    explainabilityEngine.recordBoost({
      pid: boost.pid,
      comm: boost.comm,
      schedClass: boost.schedClass,
      cores: boost.cores,
      nice: boost.nice,
      io: { ioClass: boost.ioClass, ioLevel: boost.ioLevel },
      governorChanged: CONFIG.ENABLE_GOVERNOR_SWITCH && !boost.gameModeActive && !isThermalPaused(),
      governor: CONFIG.GOVERNOR_BOOST,
      context: {
        cpuPressure: State.lastCpuPressure,
        memPressure: State.lastMemPressure,
        thermal: State.thermalLastTemp,
      },
    });
  }
}
...
function fastTick() {
  ...
  if (result.stressLevel === 'CRITICAL') {
    slowTick().catch((err) => { error('Initial slowTick error: ' + (err?.message || err)); });
  }
}
...
function _executeThrottle(action) {
  ...
  // v2.0: Record timeline event and explanation
  if (timelineEngine) {
    timelineEngine.info('scheduler', 'throttle', `Throttled ${action.comm || 'PID ' + action.pid}`, { pid: action.pid, comm: action.comm });
  }
  if (explainabilityEngine) {
    explainabilityEngine.recordThrottle({
      pid: action.pid,
      comm: action.comm,
      reason: action.reason || 'high background CPU usage',
      nice: action.nice,
      cores: action.cores,
    });
  }
...
function clearAllTimers() {
  ...
  if (intelligencePersistTimer) { clearInterval(intelligencePersistTimer); intelligencePersistTimer = null; }
  if (recommendationTimer) { clearInterval(recommendationTimer); recommendationTimer = null; }
}
...
async function setupIpcServer() {
  ...
  // ── doctor ────────────────────────────────────────────────────────
  // (doctor handler: see the v2.0 Intelligence-subsystem registration
  // further below — registerHandler overwrites by name, so that's the
  // one actually used. Removed the earlier duplicate to avoid confusion.)
  ...
  // ── v2.0: Intelligence subsystem IPC handlers ──────────────────

  // Enhanced doctor: comprehensive health check with score
  ipcServer.registerHandler('doctor', () => {
    if (doctorEngine) {
      return doctorEngine.run({
        config: CONFIG,
        selfCheckReport: State.selfCheckReport,
        daemonState: {
          stressLevel: scheduler?.stressLevel,
          foregroundPid: State.currentForegroundPid,
          throttledCount: scheduler?.autoRestore?.size || 0,
          ...
          intelligence: learningEngine ? {
            ...
            recommendationEngine: recommendationEngine ? recommendationEngine.getStatus() : null,
            explainabilityEngine: explainabilityEngine ? explainabilityEngine.getStatus() : null,
            timelineEngine: timelineEngine ? timelineEngine.getStatus() : null,
          } : null,
          monitoring: {
    ...
    // Fallback to original self-check
  ...
  // Timeline query
  ipcServer.registerHandler('timeline', (args) => {
    if (!timelineEngine) return { enabled: false, entries: [], total: 0 };
    return timelineEngine.query(args || {});
  });

  // Recommendations
  ipcServer.registerHandler('recommendations', (args) => {
    if (!recommendationEngine) return { enabled: false, pending: [] };
    if (args && args.action === 'approve' && args.id) {
      return recommendationEngine.approve(args.id);
    }
    if (args && args.action === 'dismiss' && args.id) {
      return recommendationEngine.dismiss(args.id);
    }
    if (args && args.all) {
      return { enabled: true, ...recommendationEngine.getAll({ includeExpired: true }) };
    }
    return { enabled: true, pending: recommendationEngine.getPending() };
  });

  // Explanations
  ipcServer.registerHandler('explanations', (args) => {
    if (!explainabilityEngine) return { enabled: false, entries: [] };
    return explainabilityEngine.getRecent(args || {});
  });

  // Intelligence subsystem status
  ipcServer.registerHandler('intelligence', () => {
    if (!learningEngine) return { enabled: false };
    return {
      enabled: true,
      learningEngine: learningEngine.getStatus(),
      recommendationEngine: recommendationEngine ? recommendationEngine.getStatus() : null,
  ...
  // ── v2.1.0: Tier 1 killer feature IPC handlers ───────────────────

  // Predictive Engine — predictions, top transitions, status
  ipcServer.registerHandler('predict', (args) => {
    if (!predictiveEngine) return { enabled: false };
    if (args && args.top && args.currentApp) {
      return {
        enabled: true,
        topTransitions: predictiveEngine.getTopTransitions({
          currentApp: args.currentApp,
          battery: args.battery || null,
          limit: args.limit || 5,
        }),
      };
    }
    if (args && args.currentApp) {
      const prediction = predictiveEngine.predict({
        currentApp: args.currentApp,
        battery: args.battery || null,
      });
      return { enabled: true, prediction };
    }
    return { enabled: true, ...predictiveEngine.getStatus() };
  });

  // Energy Accountant — per-app joules, system watts, totals
  ipcServer.registerHandler('energy', (args) => {
    if (!energyAccountant) return { enabled: false };
    if (args && args.apps) {
      return {
        enabled: true,
        apps: energyAccountant.getTopApps(args.limit || 20),
      };
    }
    if (args && args.history) {
      return {
        enabled: true,
        recent: energyAccountant.getRecentWatts(args.limit || 60),
      };
    }
    return { enabled: true, ...energyAccountant.getStatus() };
  });

  // VRAM Reclaim Controller — status, force-release
  ipcServer.registerHandler('vram', (args) => {
    if (!vramReclaimController) return { enabled: false };
    if (args && args.forceRelease) {
      vramReclaimController.forceRelease();
      return { enabled: true, released: true };
    }
    return { enabled: true, ...vramReclaimController.getStatus() };
  });

  // Power Budget Arbiter — status, force-release
  ipcServer.registerHandler('powerbudget', (args) => {
    if (!powerBudgetArbiter) return { enabled: false };
    if (args && args.forceRelease) {
      powerBudgetArbiter.forceRelease();
      return { enabled: true, released: true };
    }
    return { enabled: true, ...powerBudgetArbiter.getStatus() };
  });

  // HTML report generation
  ipcServer.registerHandler('report', () => {
    if (!reportGenerator || !IntelligenceModule) {
      return { error: 'intelligence subsystem not enabled' };
    }
    try {
      const doctorResult = doctorEngine ? doctorEngine.run({
        config: CONFIG,
        selfCheckReport: State.selfCheckReport,
        daemonState: {
          stressLevel: scheduler?.stressLevel,
      ...
      const html = reportGenerator.generate({
        ...
        policyEngine: policyEngine ? policyEngine.getStatus() : null,
        plugins: pluginMgr?.registeredPlugins || [],
        explanations: explainabilityEngine ? explainabilityEngine.getRecent({ limit: 50 }) : [],
        timeline: timelineEngine ? timelineEngine.exportData(200) : [],
        recommendations: recommendationEngine ? recommendationEngine.getPending() : [],
        intelligence: learningEngine ? {
          learningEngine: learningEngine.getStatus(),
          recommendationEngine: recommendationEngine ? recommendationEngine.getStatus() : null,
          explainabilityEngine: explainabilityEngine ? explainabilityEngine.getStatus() : null,
...
function bootstrap() {
  ...
  // 2. Banner
  info('='.repeat(60));
  info('DynAlloc v2.1 — Adaptive Linux Resource Manager');
  info('='.repeat(60));
  ...
  // GPU) into a unified entry point for the Policy Engine.
  //
  ...
  // (setThermalProfile, setPowerProfile, setGpuProfile) can route
  // through it.
  ...
  // setPowerProfile, setGpuProfile, setPpdProfile) can route through it. This is
  ...
  // 16k. v2.0: Intelligence Subsystem (optional)
  if (CONFIG.ENABLE_INTELLIGENCE) {
    try {
      IntelligenceModule = require('./intelligence');
      const { DoctorEngine } = require('./intelligence/doctor-engine');
      const { ReportGenerator } = require('./intelligence/report-generator');

      if (CONFIG.TIMELINE_ENABLED !== false) {
        timelineEngine = new IntelligenceModule.TimelineEngine({ config: CONFIG });
        timelineEngine.info('daemon', 'started', `DynAlloc v2.0 daemon started (PID ${SELF_PID})`);
      }

      const learningDataDir = CONFIG.LEARNING_DATA_DIR || undefined;
      learningEngine = new IntelligenceModule.LearningEngine({ config: CONFIG, dataDir: learningDataDir });
      info(`Intelligence: Learning Engine aktif (${learningEngine._totalObservations} prior observations).`);

      recommendationEngine = new IntelligenceModule.RecommendationEngine({ learningEngine, config: CONFIG });
      info('Intelligence: Recommendation Engine aktif.');

      if (CONFIG.EXPLAINABILITY_ENABLED !== false) {
        explainabilityEngine = new IntelligenceModule.ExplainabilityEngine({ config: CONFIG });
        explainabilityEngine.setContextProvider(() => ({
          stressLevel: scheduler?.stressLevel,
          cpuPressure: State.lastCpuPressure,
          memPressure: State.lastMemPressure,
          battery: null,
        ...
        info('Intelligence: Explainability Engine aktif.');
      }

      doctorEngine = new DoctorEngine();
      info('Intelligence: Doctor Engine aktif.');

      reportGenerator = new ReportGenerator();
      info('Intelligence: Report Generator aktif.');

      intelligencePersistTimer = setInterval(() => {
        if (shuttingDown) return;
        try { learningEngine.persist(); } catch (_) { /* non-fatal */ }
      }, CONFIG.INTELLIGENCE_PERSIST_INTERVAL_MS || 300000);
      if (typeof intelligencePersistTimer.unref === 'function') intelligencePersistTimer.unref();

      recommendationTimer = setInterval(() => {
        if (shuttingDown || !recommendationEngine) return;
        try {
          const fgInfo = State.currentForegroundPid ? getProcessInfo(State.currentForegroundPid) : null;
          const newRecs = recommendationEngine.generate({
            foregroundComm: fgInfo ? fgInfo.comm : null,
            stressLevel: scheduler?.stressLevel,
          });
          for (const rec of newRecs) {
            if (timelineEngine) timelineEngine.info('learning', 'recommendation', rec.summary, { id: rec.id });
            info(`Intelligence: Recommendation — ${rec.summary}`);
          }
        } catch (_) { /* non-fatal */ }
      }, CONFIG.INTELLIGENCE_RECOMMENDATION_INTERVAL_MS || 60000);
      if (typeof recommendationTimer.unref === 'function') recommendationTimer.unref();

      info('Intelligence Subsystem aktif (learning + recommendations + explainability + timeline + doctor + report).');
    } catch (err) {
      warn(`Intelligence Subsystem init error: ${err.message}`);
      learningEngine = null; recommendationEngine = null;
      explainabilityEngine = null; timelineEngine = null;
      doctorEngine = null; reportGenerator = null; IntelligenceModule = null;
    }
  }

  // 16l. v2.1.0: Tier 1 killer features — Predictive Engine,
  //       Energy Accountant, Power Budget Arbiter, VRAM Reclaim.
  //       All opt-in (default OFF). All degrade gracefully if their
  //       dependencies (LearningEngine, GpuController, etc.) are missing.
  try {
    // ── Predictive Pre-Allocation Engine ─────────────────────────────
    if (CONFIG.ENABLE_PREDICTIVE_ENGINE) {
      const { PredictiveEngine } = require('./intelligence/predictive-engine');
      predictiveEngine = new PredictiveEngine({
        config: CONFIG,
        learningEngine,
        timelineEngine,
        preAllocateFn: (pred) => {
          // Pre-warm: switch governor to performance + GPU profile to performance
          if (timelineEngine) {
            timelineEngine.info('learning', 'pre_allocate',
              `Pre-warming for "${pred.predictedApp}" (confidence ${(pred.confidence * 100).toFixed(1)}%)`);
          }
          // Capture snapshot of current governor / GPU state for rollback
          const snapshot = {
            governorBoosted: false,
            gpuProfileApplied: false,
            engagedAt: Date.now(),
          };
          try {
            if (governorMgr && CONFIG.ENABLE_GOVERNOR_SWITCH) {
              governorMgr.setGovernor(scheduler.allCores, CONFIG.GOVERNOR_BOOST, CONFIG);
              snapshot.governorBoosted = true;
            }
            if (resourceControllerMgr && CONFIG.ENABLE_GPU_CONTROL) {
              const r = resourceControllerMgr.applyGpuProfile('performance');
              snapshot.gpuProfileApplied = r.success;
            }
          } catch (_) { /* non-fatal */ }
          return snapshot;
        },
        revertFn: (snapshot) => {
          // Revert pre-allocation: restore governor to powersave + GPU to balanced
          try {
            if (snapshot.governorBoosted && governorMgr && CONFIG.ENABLE_GOVERNOR_SWITCH) {
              governorMgr.setGovernor(scheduler.allCores, 'powersave', CONFIG);
            }
            if (snapshot.gpuProfileApplied && resourceControllerMgr && CONFIG.ENABLE_GPU_CONTROL) {
              resourceControllerMgr.applyGpuProfile('balanced');
            }
          } catch (_) { /* non-fatal */ }
        },
      });
      info(`Predictive Engine aktif (threshold=${(predictiveEngine._threshold * 100).toFixed(0)}%, hold=${predictiveEngine._holdMs}ms).`);
    }

    // ── Per-App Energy Accountant (RAPL) ────────────────────────────
    if (CONFIG.ENABLE_ENERGY_ACCOUNTING) {
      const { EnergyAccountant } = require('./lib/energy-accountant');
      energyAccountant = new EnergyAccountant({ config: CONFIG });
      const ok = energyAccountant.setup();
      if (ok) {
        const tickMs = CONFIG.ENERGY_ACCOUNTING_TICK_MS || 1000;
        energyAccountantTimer = setInterval(() => {
          if (shuttingDown || !energyAccountant) return;
          try {
            // Read /proc/[pid]/stat for CPU time share attribution
            const procs = [];
            let totalCpuTicks = 0;
            const entries = fs.readdirSync('/proc').filter((e) => /^\d+$/.test(e));
            for (const e of entries.slice(0, 500)) {  // cap at 500 procs for performance
              try {
                const stat = fs.readFileSync(`/proc/${e}/stat`, 'utf8').split(' ');
                const comm = stat[1].replace(/^\(|\)$/g, '');
                const utime = parseInt(stat[13], 10) || 0;
                const stime = parseInt(stat[14], 10) || 0;
                procs.push({ pid: parseInt(e, 10), comm, utime, stime });
                totalCpuTicks += utime + stime;
              } catch (_) { /* process exited */ }
            }
            energyAccountant.sample({ processes: procs, totalCpuTicks });
          } catch (_) { /* non-fatal */ }
        }, tickMs);
        if (typeof energyAccountantTimer.unref === 'function') energyAccountantTimer.unref();
        info(`Energy Accountant aktif (${energyAccountant._vendor}, ${energyAccountant._domains.join('/')}).`);
      } else {
        warn('Energy Accountant: no RAPL/hwmon path readable — feature disabled.');
        energyAccountant = null;
      }
    }

    // ── CPU+GPU Power Budget Arbiter ────────────────────────────────
    if (CONFIG.ENABLE_POWER_BUDGET_ARBITER && resourceControllerMgr) {
      const PowerBudgetArbiter = require('./lib/controllers/power-budget-arbiter');
      const gpuController = resourceControllerMgr.getController('gpu');
      const cpuController = resourceControllerMgr.getController('cpu');
      powerBudgetArbiter = new PowerBudgetArbiter(
        { config: CONFIG, logger, cgroupManager: actuator?._cgroupManager || null, tracker: { log() {} } },
        { energyAccountant, gpuController, cpuController }
      );
      if (powerBudgetArbiter.isAvailable()) {
        resourceControllerMgr.registerController(powerBudgetArbiter);
        powerBudgetArbiter.start();
        info(`Power Budget Arbiter aktif (budget=${CONFIG.POWER_BUDGET_TOTAL_WATTS}W).`);
      } else {
        info('Power Budget Arbiter: prerequisites not met (need EnergyAccountant + GpuController available) — inert.');
      }
    }

    // ── GPU VRAM Pressure Reclaim Controller ────────────────────────
    if (CONFIG.ENABLE_VRAM_RECLAIM && resourceControllerMgr) {
      const VramReclaimController = require('./lib/controllers/vram-reclaim-controller');
      vramReclaimController = new VramReclaimController(
        { config: CONFIG, logger, cgroupManager: actuator?._cgroupManager || null, tracker: { log() {} } },
        { foregroundProvider: () => State.currentForegroundPid }
      );
      if (vramReclaimController.isAvailable()) {
        resourceControllerMgr.registerController(vramReclaimController);
        vramReclaimController.start();
        info(`VRAM Reclaim Controller aktif (vendor=${vramReclaimController._vendor}).`);
      } else {
        info('VRAM Reclaim Controller: prerequisites not met (need NVIDIA or AMD GPU control enabled) — inert.');
      }
    }
  } catch (err) {
    warn(`v2.1.0 Tier 1 features init error: ${err.message}`);
    // Don't null them out individually — bootstrap continues. The
    // individual try/catch above already handles per-feature failures.
  }
...
function cleanupAndExit(signal, exitCode = 0) {
  ...
  try {
    ...
    // Close file watcher BEFORE clearing timers (watcher may need timers to flush)
    closeFileWatcher();

    // Cleanup learning logger and per-app profiles before timer cleanup
    if (learningLogger) { try { if (typeof learningLogger.close === 'function') learningLogger.close(); } catch (_) {} learningLogger = null; }
    if (perAppProfiles) { try { if (typeof perAppProfiles.destroy === 'function') perAppProfiles.destroy(); } catch (_) {} perAppProfiles = null; }
    ...
    // v2.0: persist learning data and destroy intelligence subsystem
    if (intelligencePersistTimer) {
      clearInterval(intelligencePersistTimer); intelligencePersistTimer = null;
    }
    if (recommendationTimer) {
      clearInterval(recommendationTimer); recommendationTimer = null;
    }
    if (learningEngine) {
      try { learningEngine.persist(); } catch (_) { /* noop */ }
      learningEngine = null;
    }
    recommendationEngine = null;
    explainabilityEngine = null;
    timelineEngine = null;
    doctorEngine = null;
    reportGenerator = null;
    IntelligenceModule = null;

    // v2.1.0: Tier 1 features cleanup — revert any active pre-allocations,
    // resume any suspended PIDs, release any active power caps BEFORE
    // stopping the resource controller manager.
    if (predictiveEngine) {
      try { predictiveEngine.forceRevert(); } catch (_) { /* noop */ }
      predictiveEngine = null;
    }
    if (energyAccountantTimer) {
      clearInterval(energyAccountantTimer); energyAccountantTimer = null;
    }
    energyAccountant = null;
    if (powerBudgetArbiter) {
      try { powerBudgetArbiter.stop(); powerBudgetArbiter.destroy(); } catch (_) { /* noop */ }
      powerBudgetArbiter = null;
    }
    if (vramReclaimController) {
      try { vramReclaimController.stop(); vramReclaimController.destroy(); } catch (_) { /* noop */ }
      vramReclaimController = null;
    }
    prevForegroundComm = null;
...
function getState() {
  return {
    ...
    // v2.1.0: Tier 1 killer features
    predictiveEngine: predictiveEngine ? predictiveEngine.getStatus() : null,
    energyAccountant: energyAccountant ? energyAccountant.getStatus() : null,
    powerBudgetArbiter: powerBudgetArbiter ? powerBudgetArbiter.getStatus() : null,
    vramReclaimController: vramReclaimController ? vramReclaimController.getStatus() : null,
  };
}
