...
class ActionExecutor {
  ...
  constructor(deps) {
    ...
    // setGpuProfile) route through it. When null, those actions
    // return an error.
  ...
  async execute(action, ctx = {}) {
    ...
    try {
      switch (action.type) {
        ...
        case 'setGpuProfile':     result = await this._setGpuProfile(action); break;
        case 'setPpdProfile':     result = await this._setPpdProfile(action); break;
        // v2.1.0: new action types for Tier 1 features
        case 'setPowerBudget':    result = await this._setPowerBudget(action); break;
        case 'reclaimVram':       result = await this._reclaimVram(action); break;
        case 'preAllocate':       result = await this._preAllocate(action); break;
        default:
  ...
  async _setThermalProfile(action) {
    ...
    const result = await this._rcm.applyThermalProfile(action.profile);
    return {
  ...
  async _setPowerProfile(action) {
    ...
    const result = await this._rcm.applyPowerProfile(action.profile);
    return {
      success: result.success,
      error: result.error || null,
      profile: result.profile || null,
  ...
  /**
   * Apply a GPU profile via the ResourceControllerManager.
   * Action shape: { "type": "setGpuProfile", "profile": "performance" }
   * Valid profiles: 'balanced', 'power-saver', 'performance'
   * Requires ENABLE_GPU_CONTROL=true and a supported GPU vendor
   * (nvidia or amd) — otherwise returns { success: false }.
   */
  async _setGpuProfile(action) {
    if (!this._rcm) {
      return { success: false, error: 'Resource Controller Layer not enabled (set ENABLE_RESOURCE_CONTROLLER_LAYER=true)' };
    }
    if (typeof action.profile !== 'string' || action.profile.length === 0) {
      return { success: false, error: 'setGpuProfile.profile missing' };
    }
    const result = await this._rcm.applyGpuProfile(action.profile);
    return {
      success: result.success,
      error: result.error || null,
      profile: result.profile || null,
  ...
  async _setPpdProfile(action) {
    ...
    const result = await this._rcm.setPpdProfile(action.profile);
    return {
      success: result.success,
      error: result.error || null,
      profile: result.profile || null,
  ...
  // ── v2.1.0: Tier 1 killer feature action implementations ──────────

  /**
   * Force-engage or force-release a CPU+GPU shared power budget cap.
   * Action shape:
   *   { "type": "setPowerBudget", "op": "release" }
   *   { "type": "setPowerBudget", "op": "engage" }  (no-op if already over budget)
   *   { "type": "setPowerBudget", "op": "setBudget", "watts": 55 }
   */
  async _setPowerBudget(action) {
    if (!this._rcm) {
      return { success: false, error: 'Resource Controller Layer not enabled' };
    }
    const arbiter = this._rcm.getController('power-budget');
    if (!arbiter) {
      return { success: false, error: 'power-budget arbiter not registered (set ENABLE_POWER_BUDGET_ARBITER=true)' };
    }
    const op = action.op || 'status';
    if (op === 'release') {
      arbiter.forceRelease();
      return { success: true, op };
    }
    if (op === 'engage') {
      // Force a tick — but the arbiter's _tick() is private; we expose
      // forceRelease() as the public lever. Engage is informational here.
      return { success: true, op, note: 'engage is automatic on next tick if over budget' };
    }
    if (op === 'setBudget' && typeof action.watts === 'number') {
      // Mutate the config value at runtime; the arbiter's next setConfig
      // call will pick it up.
      if (this._config) this._config.POWER_BUDGET_TOTAL_WATTS = action.watts;
      arbiter.setConfig(this._config);
      return { success: true, op, watts: action.watts };
    }
    return { success: true, op: 'status', status: arbiter.getStatus() };
  }

  /**
   * Force-trigger VRAM reclaim NOW (regardless of watermark), or
   * release any suspended PIDs immediately.
   * Action shape:
   *   { "type": "reclaimVram", "op": "release" }
   *   { "type": "reclaimVram", "op": "status" }
   */
  async _reclaimVram(action) {
    if (!this._rcm) {
      return { success: false, error: 'Resource Controller Layer not enabled' };
    }
    const ctrl = this._rcm.getController('vram-reclaim');
    if (!ctrl) {
      return { success: false, error: 'vram-reclaim controller not registered (set ENABLE_VRAM_RECLAIM=true)' };
    }
    const op = action.op || 'status';
    if (op === 'release') {
      ctrl.forceRelease();
    ...
    return { success: true, op: 'status', status: ctrl.getStatus() };
  }

  /**
   * Manually trigger a predictive pre-allocation for a specific app,
   * bypassing the confidence threshold. Useful for testing / scripting.
   * Action shape:
   *   { "type": "preAllocate", "currentApp": "discord" }
   */
  async _preAllocate(action) {
    if (!this._config || !this._config.ENABLE_PREDICTIVE_ENGINE) {
      return { success: false, error: 'predictive engine not enabled (set ENABLE_PREDICTIVE_ENGINE=true)' };
    }
    // The actual predictive engine instance is owned by the daemon's
    // module scope. The ActionExecutor doesn't have a direct handle,
    // so this action returns guidance — the daemon's IPC `predict`
    // command is the canonical entry point.
    return {
      success: false,
      error: 'preAllocate action is informational — use `dynalloc predict` IPC command to inspect predictions',
    };
  }
