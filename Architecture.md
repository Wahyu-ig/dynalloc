...
Thermal, Power, GPU) into a single `ResourceControllerManager` entry
point.
...
```
  ...
  ┌────┴────┬────────┬────────┬──────────┬─────────┬─────────┬───────┐
  ↓         ↓        ↓        ↓          ↓         ↓         ↓       ↓
CPU      Memory    IO    Network    Governor  Thermal    Power    GPU
  ↓         ↓        ↓        ↓          ↓         ↓         ↓       ↓
                  Operating System (sysfs, cgroups, tc, nft, cpupower, gdbus, nvidia-smi)
```
...
`setGpuProfile`, `setPpdProfile`) route through it.
...
| `lib/controllers/thermal-controller.js` | Thermal profile presets + pause/resume (existing) |
| `lib/controllers/power-controller.js` | Power profile presets + PPD coordination (existing) |
| `lib/controllers/gpu-controller.js` | GPU power/performance profile presets, NVIDIA + AMD (NEW) |
...
| `setGpuProfile` | `{ "type": "setGpuProfile", "profile": "performance" }` | `GpuController.applyProfile()` |
...
Valid GPU profiles: `balanced`, `power-saver`, `performance`
(requires `ENABLE_GPU_CONTROL=true` AND an NVIDIA or AMD GPU — Intel
and systems without a discrete/supported GPU stay read-only via
`sensor.js#getGpuUtilization`, same as before this controller existed)
Valid PPD profiles: `power-saver`, `balanced`, `performance`

### GPU Controller detail

`GpuController` reuses the existing vendor probe from
`sensor.js#getGpuUtilization` (already used for read-only GPU-bound
workload classification) so vendor detection stays consistent across
the daemon. It adds a write path on top of that existing awareness:

| Vendor | Mechanism | balanced | power-saver | performance |
|---|---|---|---|---|
| NVIDIA | `nvidia-smi -pl <watts>` | card's `power.default_limit` | ~60% of `power.max_limit` | `power.max_limit` |
| AMD | `power_dpm_force_performance_level` sysfs write | `auto` | `low` | `high` |
| Intel | — (no safe universal control surface) | unavailable | unavailable | unavailable |

`GPU_USE_SUDO` (default `false`) optionally prefixes the NVIDIA
command with `sudo -n`, mirroring `GOVERNOR_USE_SUDO`'s convention in
`governor.js`. All actions are `DRY_RUN`-aware and use
`execFileSync`/sysfs writes with no shell interpolation.
