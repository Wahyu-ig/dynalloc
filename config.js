...
const DEFAULT_CONFIG = {
  ...
  // ── v2.0: Intelligence Subsystem ──────────────────────────────────
  // Gated by ENABLE_INTELLIGENCE. When true, constructs:
  //   - LearningEngine (statistical behavior learning)
  //   - RecommendationEngine (user-approval-required suggestions)
  //   - ExplainabilityEngine (decision reasoning for all actions)
  //   - TimelineEngine (queryable event timeline)
  //   - DoctorEngine (comprehensive health check)
  //   - ReportGenerator (HTML diagnostic report)
  ENABLE_INTELLIGENCE: true,     // enabled by default in v2.0
  INTELLIGENCE_PERSIST_INTERVAL_MS: 300000, // persist learning data every 5 min
  INTELLIGENCE_RECOMMENDATION_INTERVAL_MS: 60000, // generate recommendations every 60s
  LEARNING_SUGGESTION_COOLDOWN_MS: 3600000, // 1h cooldown between repeated suggestions
  LEARNING_DATA_DIR: null,       // null = ~/.config/dynalloc/learning
  EXPLAINABILITY_ENABLED: true,  // attach explanations to all decisions
  TIMELINE_ENABLED: true,        // record all important events
  TIMELINE_MAX_ENTRIES: 10000,   // ring buffer size
  ...
  //  GPU) into a single entry point for the Policy Engine.
  //
  ...
  //  (setThermalProfile, setPowerProfile, setGpuProfile) route
  //  through it.
  // ══════════════════════════════════════════════════════════════════
  ...
  // ── GPU Controller (companion to Thermal/Power controllers) ────────
  //
  // Read-only GPU utilization awareness (sensor.js#getGpuUtilization)
  // is always available and unaffected by this flag. ENABLE_GPU_CONTROL
  // additionally allows GpuController to WRITE power/performance state:
  //   - NVIDIA: power limit via `nvidia-smi -pl <watts>`
  //   - AMD:    `power_dpm_force_performance_level` sysfs write
  //   - Intel:  no supported write path — controller stays unavailable
  // Defaults OFF to preserve backward compatibility (no daemon should
  // start touching GPU power limits without explicit opt-in).
  ENABLE_GPU_CONTROL: false,
  GPU_PROFILE_DEFAULT: 'balanced',   // 'balanced' | 'power-saver' | 'performance'
  GPU_USE_SUDO: false,               // prefix nvidia-smi with `sudo -n` (mirrors GOVERNOR_USE_SUDO)
  ...
  //  Four new subsystems that no other Linux resource daemon has:
  //    1. Predictive Pre-Allocation Engine
  //    2. Per-App Energy Accounting (RAPL)
  //    3. CPU+GPU Shared Power Budget Arbiter
  //    4. GPU VRAM Pressure Reclaim Controller
  //  All default OFF — opt-in only.
  // ══════════════════════════════════════════════════════════════════

  // ── 1. Predictive Pre-Allocation ("Crystal Ball") ──────────────────
  // Uses LearningEngine's transition history to predict the next app
  // the user is likely to launch, and pre-warms CPU governor + GPU
  // power BEFORE the app actually starts. Eliminates "first 5 second
  // stutter" — a problem no other Linux resource daemon solves.
  ENABLE_PREDICTIVE_ENGINE: false,
  PREDICTION_CONFIDENCE_THRESHOLD: 0.45,     // 0-1, higher = more conservative
  PREDICTION_HOLD_MS: 8000,                  // pre-allocation auto-revert after this
  PREDICTION_COOLDOWN_MS: 15000,             // cooldown between pre-allocations

  // ── 2. Per-App Energy Accounting via RAPL ──────────────────────────
  // Reads Intel/AMD RAPL counters and attributes joules to PIDs based
  // on CPU time share. Output: "Discord: 47 Wh this week, Chrome: 89 Wh."
  // No other Linux daemon does per-app energy attribution today.
  ENABLE_ENERGY_ACCOUNTING: false,
  ENERGY_ACCOUNTING_TICK_MS: 1000,           // sampling interval (1s default)

  // ── 3. CPU+GPU Shared Power Budget Arbiter ─────────────────────────
  // Treats CPU+GPU as ONE power budget. When total draw exceeds the
  // budget, decides who to throttle based on utilization (GPU-bound vs
  // CPU-bound). All other daemons manage CPU and GPU independently.
  ENABLE_POWER_BUDGET_ARBITER: false,
  POWER_BUDGET_TOTAL_WATTS: 65,              // conservative laptop default
  POWER_BUDGET_CPU_MIN_WATTS: 8,             // floor when CPU is being capped
  POWER_BUDGET_GPU_MIN_WATTS: 10,            // floor when GPU is being capped
  ...
  POWER_BUDGET_TICK_MS: 2000,                // re-evaluation interval

  // ── 4. GPU VRAM Pressure Reclaim Controller ────────────────────────
  // Monitors VRAM usage. When > 92% AND foreground needs VRAM, briefly
  // SIGSTOPs the largest non-foreground VRAM hogs so the driver reclaims
  // their VRAM. Prevents GPU OOM crashes that plague Linux gaming.
  ENABLE_VRAM_RECLAIM: false,
  VRAM_RECLAIM_TICK_MS: 5000,                // scan interval
  VRAM_RECLAIM_HIGH_WATERMARK: 0.92,         // 92% used → reclaim
  VRAM_RECLAIM_LOW_WATERMARK: 0.80,          // 80% used → clear pressure state
...
const CONFIG_SCHEMA = {
  ...
  INTELLIGENCE_PERSIST_INTERVAL_MS: { type: 'number', min: 10000, max: 3600000, integer: true },
  INTELLIGENCE_RECOMMENDATION_INTERVAL_MS: { type: 'number', min: 10000, max: 3600000, integer: true },
  LEARNING_SUGGESTION_COOLDOWN_MS: { type: 'number', min: 60000, max: 86400000, integer: true },
  LEARNING_DATA_DIR:                { type: 'path', nullable: true },
  EXPLAINABILITY_ENABLED:           { type: 'boolean' },
  TIMELINE_ENABLED:                 { type: 'boolean' },
  TIMELINE_MAX_ENTRIES:             { type: 'number', min: 100, max: 100000, integer: true },
  ...
  // GPU Controller
  ENABLE_GPU_CONTROL: { type: 'boolean' },
  GPU_PROFILE_DEFAULT: { type: 'string' },  // validated against VALID_PROFILES at runtime
  GPU_USE_SUDO:        { type: 'boolean' },
  ...
  // v2.1.0: Predictive Pre-Allocation Engine
  ENABLE_PREDICTIVE_ENGINE:          { type: 'boolean' },
  PREDICTION_CONFIDENCE_THRESHOLD:   { type: 'number', min: 0, max: 1 },
  PREDICTION_HOLD_MS:                { type: 'number', min: 1000, max: 60000 },
  PREDICTION_COOLDOWN_MS:            { type: 'number', min: 1000, max: 300000 },

  // v2.1.0: Per-App Energy Accounting (RAPL)
  ENABLE_ENERGY_ACCOUNTING:          { type: 'boolean' },
  ENERGY_ACCOUNTING_TICK_MS:         { type: 'number', min: 500, max: 60000 },

  // v2.1.0: CPU+GPU Shared Power Budget Arbiter
  ENABLE_POWER_BUDGET_ARBITER:       { type: 'boolean' },
  POWER_BUDGET_TOTAL_WATTS:          { type: 'number', min: 10, max: 1000 },
  POWER_BUDGET_CPU_MIN_WATTS:        { type: 'number', min: 1, max: 100 },
  POWER_BUDGET_GPU_MIN_WATTS:        { type: 'number', min: 1, max: 100 },
  POWER_BUDGET_HEADROOM_FACTOR:      { type: 'number', min: 0.5, max: 0.99 },
  POWER_BUDGET_RELEASE_FACTOR:       { type: 'number', min: 1.01, max: 2.0 },
  POWER_BUDGET_TICK_MS:              { type: 'number', min: 500, max: 60000 },

  // v2.1.0: GPU VRAM Pressure Reclaim
  ENABLE_VRAM_RECLAIM:               { type: 'boolean' },
  VRAM_RECLAIM_TICK_MS:              { type: 'number', min: 1000, max: 60000 },
  VRAM_RECLAIM_HIGH_WATERMARK:       { type: 'number', min: 0.5, max: 0.99 },
  VRAM_RECLAIM_LOW_WATERMARK:        { type: 'number', min: 0.3, max: 0.95 },
  VRAM_RECLAIM_HOLD_MS:              { type: 'number', min: 500, max: 30000 },
  VRAM_RECLAIM_COOLDOWN_MS:          { type: 'number', min: 1000, max: 300000 },
  VRAM_RECLAIM_MAX_PER_TICK:         { type: 'number', min: 1, max: 10, integer: true },
...
const HOT_RELOADABLE_FIELDS = [
  ...
  'ENABLE_GPU_CONTROL', 'GPU_PROFILE_DEFAULT', 'GPU_USE_SUDO',
  ...
  // v2.0: Intelligence subsystem
  'ENABLE_INTELLIGENCE', 'INTELLIGENCE_PERSIST_INTERVAL_MS',
  'INTELLIGENCE_RECOMMENDATION_INTERVAL_MS',
  'LEARNING_SUGGESTION_COOLDOWN_MS', 'LEARNING_DATA_DIR',
  'EXPLAINABILITY_ENABLED', 'TIMELINE_ENABLED', 'TIMELINE_MAX_ENTRIES',
  'CPU_HISTORY_SIZE',
  ...
  // v2.1.0: Tier 1 killer features
  'ENABLE_PREDICTIVE_ENGINE', 'PREDICTION_CONFIDENCE_THRESHOLD',
  'PREDICTION_HOLD_MS', 'PREDICTION_COOLDOWN_MS',
  'ENABLE_ENERGY_ACCOUNTING', 'ENERGY_ACCOUNTING_TICK_MS',
  'ENABLE_POWER_BUDGET_ARBITER', 'POWER_BUDGET_TOTAL_WATTS',
  'POWER_BUDGET_CPU_MIN_WATTS', 'POWER_BUDGET_GPU_MIN_WATTS',
  'POWER_BUDGET_HEADROOM_FACTOR', 'POWER_BUDGET_RELEASE_FACTOR',
  'POWER_BUDGET_TICK_MS',
  'ENABLE_VRAM_RECLAIM', 'VRAM_RECLAIM_TICK_MS',
  'VRAM_RECLAIM_HIGH_WATERMARK', 'VRAM_RECLAIM_LOW_WATERMARK',
  'VRAM_RECLAIM_HOLD_MS', 'VRAM_RECLAIM_COOLDOWN_MS',
  'VRAM_RECLAIM_MAX_PER_TICK',
];
...
function resolveConfigPath() {
  let homeDir;
  try { homeDir = os.homedir(); } catch (_) { homeDir = null; }
  const candidates = [
    process.env.DYNALLOC_CONFIG_PATH,
    path.join(homeDir || '', '.config', 'dynalloc', 'config.json'),
...
function validateField(key, value, schema, defaultVal) {
  ...
  if (schema.type === 'ioPrio') {
    ...
    if (!Number.isInteger(value[0]) || !Number.isInteger(value[1])) return defaultVal;
...
function validateAndMerge(base, override, logWarnings = false) {
  ...
  if (merged.THERMAL_RESUME_THRESHOLD >= merged.THERMAL_PAUSE_THRESHOLD) {
    if (logWarnings) warnings.push('THERMAL_RESUME_THRESHOLD >= THERMAL_PAUSE_THRESHOLD, reverting to defaults');
    merged.THERMAL_RESUME_THRESHOLD = DEFAULT_CONFIG.THERMAL_RESUME_THRESHOLD;
    merged.THERMAL_PAUSE_THRESHOLD = DEFAULT_CONFIG.THERMAL_PAUSE_THRESHOLD;
  }
