...
class ResourceControllerManager {
   ...
   * Apply a GPU profile.
   * @param {string} profileName
   * @returns {object} result from GpuController.applyProfile
   */
  applyGpuProfile(profileName) {
    const c = this.getController('gpu');
    if (!c) return { success: false, error: 'gpu controller not registered' };
    this._actionCount++;
    return c.applyProfile(profileName);
  }
  ...
  _registerBuiltins() {
    ...
    const gpu = new GpuController(deps);
    this._controllers.set('gpu', gpu);
    debug('Resource controller registered: gpu');
  }
}
